...
const command = new Command("Miner")
    ...
    .addArgument(new Argument("ContractId", "Identifier of the contract targeted"))
    .addArgument(new Argument("GoodSymbol", "Symbol of the good to buy & deliver"))
...
while (true) {
    ...
    const good = contract.terms.deliver?.find((good) => good.tradeSymbol == goodSymbol);
    if (good == undefined) {
    ...
    try {
        const navRes = (await Api.fleet.navigateShip(shipSymbol, { waypointSymbol: buyWaypoint })).data.data;
        const navDuration = new Date(navRes.nav.route.arrival).getTime() -
    ...
    if (buyUnits != 0) {
        const buyRes = (await Api.fleet.purchaseCargo(shipSymbol, {
            symbol: goodSymbol,
            units: buyUnits,
        })).data.data;
        const cargo = buyRes.cargo.inventory.find((item) => item.symbol == goodSymbol);
        if (cargo == undefined) {
            break;
        }
    }
    ...
    const navDeliverRes = (await Api.fleet.navigateShip(shipSymbol, {
        waypointSymbol: good.destinationSymbol,
    })).data.data;
    const navDeliverDuration = new Date(navDeliverRes.nav.route.arrival).getTime() -
    ...
    // Deliver
    log("Deliver");
    const cargoUnits = (await Api.fleet.getMyShipCargo(shipSymbol)).data.data.inventory.find((item) => item.symbol == goodSymbol)?.units;
