...
const mine = async (shipSymbol) => {
    ...
    const surveyMining = async () => {
        ...
        if (existsSync(surveyPath)) {
            ...
            const sortedSurveys = readdirSyncRes.sort((a, b) => +b.trim().split("-")[0] - +a.trim().split("-")[0]);
...
const store = async (shipSymbol) => {
    ...
    if (localShips.length > 0) {
        ...
        for (const cargoItem of currentShip.cargo.inventory) {
            const maxTransferingUnits = Math.min(cargoItem.units, localShips[0].cargo.capacity - localShips[0].cargo.units);
            if (maxTransferingUnits > 0) {
                try {
                    const transferCargoRes = (await Api.fleet.transferCargo(shipSymbol, {
                        shipSymbol: localShips[0].symbol,
                        tradeSymbol: cargoItem.symbol,
                        units: Math.min(cargoItem.units, localShips[0].cargo.capacity - localShips[0].cargo.units),
                    })).data.data;
