...
const waitForFullCargo = async (
  ...
  return new Promise<void>(async (resolve) => {
    ...
    if (cooldown) {
      log(
        "Ship on cooldown, should wait for " + cooldown.remainingSeconds + "s"
      );
      await new Promise((resolve) =>
        setTimeout(() => resolve(null), cooldown.remainingSeconds * 1000)
      );
    }
    ...
    const market = (
      await Api.system.getMarket(nav.systemSymbol, nav.waypointSymbol)
    ).data.data;
    ...
    if (market.tradeGoods) {
      for (const good of market.tradeGoods) {
        possibleValue +=
          (createSurveyRes.surveys[0].deposits.filter(
            (deposit) => deposit.symbol == good.symbol
          ).length *
            good.sellPrice) /
    ...
    const filename =
      possibleValue + " - " + createSurveyRes.surveys[0].signature + ".json";
...
const deliver = async (shipSymbol: string) => {
  ...
  try {
    ...
    if (contract.terms.deliver) {
      ...
      item = shipCargo.inventory.find(
        (item) => item.symbol == deliver[0].tradeSymbol
      );
    }
    const deliverRes = (
...
const sellCargo = async (shipSymbol: string) => {
  const ship = (await Api.fleet.getMyShip(shipSymbol)).data.data;
  const market = (
    await Api.system.getMarket(ship.nav.systemSymbol, ship.nav.waypointSymbol)
  ).data.data;
  ...
  for (const cargoItem of ship.cargo.inventory) {
    if (market.tradeGoods) {
      const n =
        market.tradeGoods.filter((tradeGood) => {
          return tradeGood.symbol == cargoItem.symbol;
        }).length == 1;

      log(
        (n ? "Possible" : "Not possible") + " to sell " + " " + cargoItem.symbol
      );

      if (n) {
        const sellCargoRes = (
          await Api.fleet.sellCargo(shipSymbol, {
...
const jettisonCargo = async (shipSymbol: string, keepItemSymbol?: string[]) => {
  const ship = (await Api.fleet.getMyShip(shipSymbol)).data.data;
  ...
  for (const cargoItem of filteredInventory) {
    const jettisonRes = (
      await Api.fleet.jettison(shipSymbol, {
        symbol: cargoItem.symbol,
        units: cargoItem.units,
      })
    ).data.data;
...
const goBack = async (shipSymbol: string) => {
  try {
